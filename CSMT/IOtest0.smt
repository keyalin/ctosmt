...
;(assert (forall ((s1 String) (i Int))
;		(ite
;			(and (>= i 0) (< i (length s1)))
;			(exists ((c Char)) (= (charOf s1 i) c))
;			(= (exists ((c Char)) (= (charOf s1 i) c)) false))))

			
(declare-fun b () String)
(declare-fun ing () String)
(declare-fun k () String)
(declare-fun string2 () String)
(declare-fun out () String)
(declare-fun result5 () String)
(assert (= (charOf string2 1) _a_ ))
(assert (= (charOf string2 2) _r_ ))
(assert (= out result5))
(assert (= (length result5) (+ (length k) (length b))))
(assert (>= (length k) 0))
(assert (>= (length b) 0))
(assert (= (length string2) 3))
(assert (forall ((index Int))(ite (and(>= index 0)(< index (length k)))(= (charOf result5 index) (charOf k index))(ite (and(>= index (length k))(< index (+ (length k) (length b))))(= (charOf result5 index) (charOf b (- index (length k))))true))))
(assert (>= (length string2) 0))
(assert (= k ing))
(assert (= (charOf string2 0) _b_ ))
(assert (= b string2))
(assert (>= (length result5) 0))
(declare-fun LSinput () String)
(declare-fun LSoutput () String)
(assert (= (charOf LSinput 0) _f_ ))
(assert (= (charOf LSinput 1) _o_ ))
(assert (= (charOf LSinput 2) _o_ ))
(assert (= (length LSinput) 3))
(assert (= (charOf LSoutput 0) _f_ ))
(assert (= (charOf LSoutput 1) _o_ ))
(assert (= (charOf LSoutput 2) _o_ ))
(assert (= (charOf LSoutput 3) _b_ ))
(assert (= (charOf LSoutput 4) _a_ ))
(assert (= (charOf LSoutput 5) _r_ ))
(assert (= (length LSoutput) 6))
(assert (= ing LSinput ))
(assert (= LSoutput out))
(check-sat)
(exit)
