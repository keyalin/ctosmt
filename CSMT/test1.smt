...
(assert 
	(forall ((c Char))
		(or
			...
			(= c _question_)(= c _backslash_)(= c _space_)
		)
	)
)
...
(assert (= (charOf inputString 2) _d_ ))
(assert (= (length inputString) 3))
(assert (= staticString inputString))
(check-sat)



(assert (= (charOf outputString 0) _a_ ))
(assert (= (charOf outputString 1) _b_ ))
(assert (= (charOf outputString 2) _c_ ))
(assert (= (charOf outputString 3) _g_ ))
(assert (= (charOf outputString 4) _b_ ))
(assert (= (charOf outputString 5) _c_ ))
(assert (= (length outputString) 6))


(assert (substringEqualAt outputString inputString 0))
(assert (substringEqualAt outputString inputString (length inputString)))
